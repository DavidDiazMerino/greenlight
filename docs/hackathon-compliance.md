...
Last reviewed: 2026-09-03. Deadline: **2026-09-09 14:00 PDT / 23:00 CEST (Madrid)**.
...
| Functional web, Android, or iOS project | Public Cloud Run Decision Card is functional at `https://greenlight-easase2aiq-ew.a.run.app` | PASS | Keep the URL available through judging. |
...
| Gemini and Google Cloud Agent Builder usage at runtime | `@google/adk` 2.0 `LlmAgent`/`Runner` called Vertex AI `gemini-2.5-pro` in the billing-enabled `greenlight-agentic-cinema` project; Cloud KMS signed the final receipt | PASS | Keep both live entry points reproducible and show the verification result in the demo. |
| Grafana stack actively used at runtime primarily through official Grafana MCP | Hosted `MCPToolset` used OAuth 2.1, discovered 119 tools, investigated a firing alert, returned non-empty Prometheus/Loki/Tempo evidence, confirmed the exact failing trace, found and annotated the dashboard, and generated its review link in eight receipted calls | PASS | Keep the sanitized verification record and demonstrate the alert-to-dashboard mission in the video. |
...
| Hosted project URL | `https://greenlight-easase2aiq-ew.a.run.app` serves the Decision Card and sanitized live verification without credentials | PASS | Recheck immediately before submission. |
| Public demo video on YouTube or Vimeo, no longer than three minutes | A 2:18 H.264/AAC master tells Avery's dependency-update story with two disclosed MiniMax H3 opening shots, documented context cards, real product UI, burned English captions, full-decode QA, and contact sheet | PARTIAL | Record/mux the timed English voiceover if desired, approve the master, then upload publicly to YouTube or Vimeo. |
| Written submission in English | Complete draft in [`../SUBMISSION.md`](../SUBMISSION.md) | PASS | Paste into Devpost and adjust only for final video/team fields. |
| Third-party integrations and media are authorized | Canary visuals are original synthetic/MIT; the H3 opening is disclosed and isolated in the rights inventory; Google/Grafana/MiniMax/Nous/FAL use must follow their terms | PARTIAL | Confirm the intended publication is covered by the provider account terms; do not add unlicensed footage, music, logos, slogans, or third-party personal data. |

## Runtime proof that must exist in the final repository

Devpost explicitly requires Google Cloud and partner services to be imported and actually called in code, not merely named in documentation. Items 1–6 now have repository or credentialed runtime evidence:
...
4. Real Prometheus, Loki, and Tempo queries retrieved through MCP and attached to the Decision Card with exact tool/query/result hashes. Raw results are re-hashed before the MCP-required policy evaluation; the hardened proof is [`verification/live-alert-dashboard-mcp-2026-09-03.json`](verification/live-alert-dashboard-mcp-2026-09-03.json).
5. The deterministic policy must remain the sole verdict owner; Gemini may select tests, investigate the alert, explain results, and annotate the dashboard, but must not alter thresholds or override `PROMOTE`, `HOLD`, or `REJECT`.
...
7. A genuine Cloud KMS `EC_SIGN_P256_SHA256` key version signs the final Decision Receipt; the public envelope and key can be verified offline with `npm run receipt:verify`.
...
- scope: read access for evidence and `grafana:write` only for the versioned dashboard, alert, and annotation workflow.
...
2. require `alerting_manage_rules` to return the named safe-area rule in `firing` state;
3. execute two exact `query_prometheus` calls for baseline/candidate render duration;
4. execute one exact `query_loki_logs` call for experiment-scoped structured media-QA records;
5. execute one exact `tempo_traceql-search` call for experiment-scoped traces, including a paired baseline/candidate clip;
6. use `search_dashboards` to verify the versioned dashboard UID;
7. use `create_annotation` to attach the immutable verdict, Decision Receipt fingerprint, and exact highest-failure clip/overflow/trace ID;
8. use `generate_deeplink` to produce a same-stack, experiment-scoped review URL;
9. reject altered calls or empty results, re-hash raw evidence proofs, separate evidence/workflow receipts, and preserve the committed policy verdict.
...
- **Potential impact:** make the cost of a deceptively green dependency update concrete for Avery Morgan, the fictional Media Platform Lead responsible for a high-volume campaign render queue.
